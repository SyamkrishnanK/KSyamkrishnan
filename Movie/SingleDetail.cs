...
namespace Movie
{
    public abstract class SingleDetail : ISingleDetail
    {
        ...
        static Dictionary<string, Func<XmlNode, ISingleDetail>> _dict = new Dictionary<string, Func<XmlNode, ISingleDetail>>();
        ...
        public static ISingleDetail ReadSpec(XmlNode xSpec)
        {
            ...
            if (!string.IsNullOrEmpty(stType) && _dict.ContainsKey(stType.ToLower()))
                return _dict[stType](xSpec);
            return StringDetail.CreateStringDetail(xSpec);
        }
        ...
        protected virtual void Parse(XmlNode xNode)
        {
